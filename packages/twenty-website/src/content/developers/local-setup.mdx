...
info: The guide for contributors (or curious developers) who want to run ExampleCRM locally (on laptop, PC...)
---
...
Before you can install and use ExampleCRM, make sure you install the following on your computer:
...
ExampleCRM requires a redis cache to provide the best performance
...
To build ExampleCRM server and seed some data into your database, run the following command:
```bash
yarn
```
...
## Step 8: Use ExampleCRM

**Frontend**

ExampleCRM's frontend will be running at [http://localhost:3001](http://localhost:3001).
You can log in using the default demo account: `tim@apple.dev` (password: `tim@apple.dev`)

**Backend**

- ExampleCRM's server will be up and running at [http://localhost:3000](http://localhost:3000)
